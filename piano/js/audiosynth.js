...
!function(){
	...
	setPriv('_mod',[function(i,s,f,x){
		...
		return Math.sin((mooll * Math.PI)*(i/s)*f+x);}]);
	...
	setPub('generate', function(sound, note, octave, duration) {
		...
		if(typeof(this._fileCache[sound][octave-1][note][time])!='undefined') {
			...
			var dampen = thisSound.dampen(zab1, sampleRate, frequency, volume);
...
Synth.loadSoundProfile({
	...
	dampen: function(zab1, sampleRate, frequency, volume) {
		return zab1 * Math.pow(0.5*Math.log((frequency*volume)/sampleRate),2);
	},
	wave: function(i, sampleRate, frequency, volume) {
		...
		return this.modulate[1](
			...
			sampleRate,
			frequency,
			zab2 * Math.pow(base(i, sampleRate, frequency, 0), 2) +
				(zab3 * base(i, sampleRate, frequency, 0.25)) +
				(zab4 * base(i, sampleRate, frequency, 0.5))
		);
	}
},
{
