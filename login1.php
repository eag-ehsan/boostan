...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $sql = "SELECT ID, username, email, password, nDate, nTime, fname, lname, cHash, validate, userclass, nPostnum, nPostview, nPostreply, nCity, nHaby, nBlog, userIMG, nWsite, nActivate, nEhraz, pTaklif, pAzmoon, pTalking FROM users WHERE username='" . $_POST['username'] . "'";
    ...
    else
    {
      ...
      $_SESSION["HID_username"] = $_POST['username'];
      $_SESSION["HID_fname"] = $row['fname'];
      $_SESSION["HID_lname"] = $row['lname'];
      $_SESSION["HID_email"] = $row['email'];
      $_SESSION["HID_userIMG"] = $row['userIMG'];
      $_SESSION["HID_userclass"] = $row['userclass'];
      $_SESSION["HID_nPostreply"] = $row['nPostreply'];
      $_SESSION["HID_nPostnum"] = $row['nPostnum'];
      $_SESSION["HID_nPostview"] = $row['nPostview'];
      $_SESSION["HID_nActivate"] = $row['nActivate'];
      $_SESSION["HID_nEhraz"] = $row['nEhraz'];
      $_SESSION["HID_pTaklif"] = $row['pTaklif'];
      $_SESSION["HID_pAzmoon"] = $row['pAzmoon'];
      $_SESSION["HID_pTalking"] = $row['pTalking'];
      $_SESSION["HID_ISLOGIN"] = 1;
      $_SESSION["HID_ID"] = $row['ID'];
      $karbarName = $row['fname'] . " " . $row['lname'];
      $checkOK = 203;
      }
      else
      {
...
<?php if($continu==1){ ?>
            <form id="contact" name="contact" action="" method="post">
                ...
                <div class="kr0-divofoghi-users2">
                    ...
                    <button type="submit" name="lrnlogin" id="form-submit" class="main-button">ورود</button>
